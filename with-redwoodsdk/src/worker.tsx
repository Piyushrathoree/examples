...
const polar = new Polar({
  accessToken: cloudflareEnv?.POLAR_ACCESS_TOKEN,
  server: cloudflareEnv?.POLAR_MODE as "sandbox" | "production",
});
  ...
  prefix("/api", [
    ...
    route("/checkout", async ({ request }) => {
      ...
      const checkout = await polar.checkouts.create({
        products: [productId],    
        successUrl: cloudflareEnv?.POLAR_SUCCESS_URL || url.origin,
      });
  ...
  route("/polar/webhooks", async ({ request }) => {
    ...
    try {
      const base64Secret = Buffer.from(
        cloudflareEnv.POLAR_WEBHOOK_SECRET,
        "utf-8"
      ).toString("base64");
